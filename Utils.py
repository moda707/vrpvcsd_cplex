...
class LinearSchedule(object):
    ...
    def val(self, t):
        ...
        if t > self.end_t:
            return self.end_val
        return ((t - self.init_t) * (self.end_val - self.init_val)) / float(self.end_t - self.init_t) + self.init_val

    def update_time(self, t):
        if t < self.init_t or t > self.end_t:
            return False
...
def plot_environment(c, v, depot=(50, 50), c_id_modifier=0, service_area_length=(100, 100),
    ...
    for n in c:
        if detail_level == 0:
            ax.text(n[1], n[2], f"{n[0]:n}")
        elif detail_level == 1:
            ax.text(n[1], n[2], f"{n[0]:n} ({n[4]:.2f})")
        elif detail_level == 2:
            ax.text(n[1], n[2], f"{n[0]:n} ({n[4]:.2f}, {n[6]:.2f})")
        elif detail_level == 3:
            ax.text(n[1], n[2], f"{n[0]:n} ({n[4]:.2f}, {n[6]:.2f}, {n[6] - n[5]:.2f})")
        else:
...
def plot_customers(c, service_area=80, zone_step=5):
    ...
    for n in c:
        ...
        ax[0].text(n[1], n[2], f"{n[0]:n}, {n[4]}")

    plt.show()


class TestResults:
    def __init__(self, final_reward=0, actions=None, n_routes=0, n_fully_served=0, n_partially_served=0, n_visits=0,
                 avg_travel_time=0, total_demand=0,
                 expected_demand=0, max_travel_time=0, agents_reward=None):
        self.final_reward = final_reward
        if actions is None:
            actions = []
        self.actions = actions
        self.n_routes = n_routes
        self.n_fully_served = n_fully_served
        self.n_partially_served = n_partially_served
        ...
        if agents_reward is None:
            agents_reward = []
        self.agent_reward = agents_reward
        self.tot_realized_demand = 0.
        self.service_rate = 0.
        self.n_route_failure = 0

    def print_full(self, title=False):
        if title:
            print("FR", "#routes", "#fully_served", "#partially_served", "#visits", "#avg_tt", "#max_tt", "#preemp", "exp_dem")
        print(round(self.final_reward, 2), self.n_routes, self.n_fully_served, self.n_partially_served, self.n_visits,
    ...
    def accumulate_results(self, results):
        ...
        self.n_fully_served += results.n_fully_served
        self.n_partially_served += results.n_partially_served
        ...
        self.n_route_failure += results.n_route_failure

    def print_avgs_full(self, title=False):
        if title:
            print("FR", "#routes", "#fully_served", "#partially_served", "#visits", "#avg_tt", "#max_tt", "#preemp",
                  "tot_dem", "service rate")
        print(f"{self.final_reward / self.results_count:.2f}\t"
              f"{self.n_routes / self.results_count:.2f}\t"
              f"{self.n_fully_served / self.results_count:.2f}\t"
              f"{self.n_partially_served / self.results_count:.2f}\t"
              f"{self.n_visits / self.results_count:.2f}\t"
              f"{self.avg_travel_time / self.results_count:.2f}\t"
              f"{self.max_travel_time / self.results_count:.2f}\t"
              f"{self.n_preemptives / self.results_count:.2f}\t",
              f"{self.n_route_failure / self.results_count:.2f}\t",
              f"{self.tot_realized_demand / self.results_count:.2f}\t",
              f"{self.service_rate * 100. / self.results_count:.2f}\t"
              )

    def full2(self, title=False):
        res_str = f"{self.expected_demand * 25:.2f}\t" \
                  f"{self.final_reward:.2f}\t{self.n_routes:.2f}\t" \
                  f"{self.n_served:.2f}\t{self.n_visits:.2f}\t" \
                  f"{self.avg_travel_time:.2f}\t" \
                  f"{self.max_travel_time:.2f}\t" \
                  f"{self.n_preemptives:.2f}"
